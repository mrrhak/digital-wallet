...
export abstract class BaseService<T> {
  ...
  async update(params: Partial<Record<keyof T, unknown>>): Promise<T | null> {
    ...
    return await this.model.findByIdAndUpdate(id, params as UpdateQuery<T>, {
      new: true,
    });
  }
  ...
  async getPaginatedList(
    ...
    metadata: { limit: number; page: number; total: number };
  }> {
    ...
    return {
      records: records.length
        ? (records.map((v) => this.model.hydrate(v)) as any)
        : [],
      metadata: {
        limit: pageFilter.limit,
        page: pageFilter.page,
        total: t(docs, '0.pageInfo[0].total').safeNumber,
      },
    };
  }
